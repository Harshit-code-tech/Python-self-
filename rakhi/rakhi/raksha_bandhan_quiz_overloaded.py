...
from correctpass import CorrectPassProgram
...
class RakshaBandhanProgram:
    def __init__(self, root):
        ...
        self.topframe = Frame(self.root, bg="#FF0000", width=800, height=50)  # Set the background color of the fram
        self.topframe.pack(side=TOP)

        self.leftframe = Frame(self.root, width=400, height=500,
                               bg="#FF0000")  # Set the background color of the fram
        ...
        self.exit_btn = Button(self.leftframe, font=('Arial', 16, 'bold'), text="Exit", bg="#FF6347",
                               fg="#FFFFFF", bd=3, padx=10, pady=10, width=12, command=root.destroy)
        self.exit_btn.grid(row=5, column=1, padx=10, pady=10)
        self.progress_bar_widget = Progressbar(self.topframe, orient="horizontal", length=200, mode="determinate")
        self.progress_bar_widget.grid(row=0, column=2, padx=10, pady=10)
    ...
    def update_progress_bar(self):
        ...
        progress_percentage = (self.current_question_index + 1) / total_questions * 100
        self.progress_bar_widget["value"] = progress_percentage
    ...
    def send_wishes(self):
        ...
        try:
            subprocess.run(["python", "hearttry.py"])  # Adjust the command as needed
    ...
    def fade_in(self, widget, duration=1000):
        ...
        def increase_alpha():
            widget.alpha += 5
            if widget.alpha <= 255:
                widget.configure(bg=f"#{widget.alpha:02X}{widget.alpha:02X}{widget.alpha:02X}")
                self.root.after(duration // 255, increase_alpha)
    ...
    def show_results(self):
        ...
        # Update total time elapsed label
        self.update_total_time_elapsed()
        # Add fade-in effect to result_text widget
        self.fade_in(self.result_text)

        for i, (user_ans, question_data) in enumerate(zip(self.user_answers, self.quiz_data)):
            question = question_data["question"]
            is_correct = user_ans == question_data["correct_answer"]

            result_text += f"Q{i + 1}: {question} - Time: {self.times_consumed[i]} seconds\n"

            # Add visual indicators using icons or emojis
            indicator_icon = "✅" if is_correct else "❌"
            result_text += f"{indicator_icon} {'Correct' if is_correct else 'Incorrect'}\n"
...
def main_program_function():
    root_main = tk.Tk()
    app_main = RakshaBandhanProgram(root_main)
    app_main.main()
    root_main.mainloop()
...
if __name__ == "__main__":
    root = tk.Tk()
    app = CorrectPassProgram(root, main_program_function)
    root.mainloop()
